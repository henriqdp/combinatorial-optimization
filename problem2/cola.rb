...
File.open("teste.txt", "r+") do |f|
   ...
   plaintext[1] = plaintext[1].split(" ").collect{|x| x = x.to_i}
   plaintext[2] = plaintext[2].split(" ").collect{|x| x = x.to_i}

   plaintext[0] = plaintext[0].to_i
   plaintext[3] = plaintext[3].to_i
end

items = Array.new
(0..plaintext[2].size-1).each do |i|
  items << KnapsackItem.new("item_#{i}", plaintext[3][i], plaintext[2][i])
end
...
=end
  problem = KnapsackProblem.new(items, plaintext[3])
